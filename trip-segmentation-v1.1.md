# **Trip segmentation v1.1 (deterministic, jitter-aware)**

**Status**: ✅ Deployed to production (November 2025)
**Implementation**: `supabase/migrations/017_add_trip_segmentation.sql` + `scripts/process-trips.py`
**Automation**: Vercel Cron (every 6 hours) + manual Python fallback
...
3. **Automated Processing** (`app/api/cron/process-trips/route.ts`)
   - Next.js API endpoint calling `derive_trips()` SQL function
   - Vercel Cron: runs every 6 hours (00:00, 06:00, 12:00, 18:00 UTC)
   - Processes last 24 hours incrementally per vehicle
   - Secured with `CRON_SECRET` authentication
   - Average processing time: ~1 second for 4-5 vehicles
...
- Implementation: `supabase/migrations/017_add_trip_segmentation.sql`
- Python Script: `scripts/process-trips.py`
- Automation: `app/api/cron/process-trips/route.ts`
- Cron Config: `vercel.json`
- Setup Guide: `CRON_SETUP.md`
- Bug Fix: `TRIP_DISTANCE_BUG_FIX.md`
- Summary: `TRIP_PROCESSING_SUMMARY.md`

---

**Last Updated**: November 11, 2025
**Status**: Production-ready with automated processing every 6 hours
